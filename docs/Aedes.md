...
MQTT 5.0 only. Emitted when the [`pendingSessionsLimit`](#new-aedesoptions) DoS guard trips: a disconnecting session that would otherwise be retained (including a never-expiring one) is instead expired immediately, or a delayed Will is published immediately instead of being held. Listen for this to detect identity-cycling abuse — without it the guard would degrade behavior silently.
