...
- options `<object>`
  ...
  - `maxInflightInbound` `<number>` broker-side hard cap on the number of inbound QoS 2 `PUBLISH` a single client may hold in flight (stored awaiting `PUBREL`). Bounds the per-client incoming persistence map so a client that keeps sending fresh QoS 2 message identifiers while withholding `PUBREL` cannot grow it toward the 16-bit messageId ceiling. Exceeding it emits `clientError` with `maxInflightInbound exceeded: <limit> inbound QoS 2 messages already awaiting PUBREL` and drops the connection; MQTT 3.1.1 has no reason code to send instead. Counts packets, not bytes — pair it with a transport-level frame limit if you need a byte bound. `0` disables the cap and restores pre-1.1.3 behaviour, which is what GHSA-5jvp-3p2v-5qgf describes; a value that is not a non-negative integer falls back to the default rather than disabling it. __Default__: `1000`
