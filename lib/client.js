...
function Client (broker, conn, req) {
  ...
  this.deliverShared = function (_packet, cb) {
    ...
    that.broker.persistence.nextSharedSubscription(sharedSub.topic, sharedSub.group, function (err, subscription) {
      if (err) {
        ...
        if (subscription.clientId === that.id) { // this client should send an update
          that.deliverQoS(_packet, function (err) {
            if (err) {
              ...
              that.broker.persistence.updateSharedSubscription(that, sharedSub.topic, sharedSub.group, cb)
            }
          })
        }
      }
