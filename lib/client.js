...
function Client (broker, conn) {
  ...
  this.deliver1 = function deliverQoS1 (_packet, cb) {
    ...
    broker.persistence.outgoingUpdateMessageId(that, packet, writeQoS)
  }
}
...
function ConnectStatus (client, packet) {
  this.client = client
  this.packet = packet
}
...
function handleConnect (client, packet, done) {
  ...
  client.broker._series(
    new ConnectStatus(client, packet),
    [ fetchSubs,
      restoreSubs,
      doConnack,
      emptyQueue
...
function emptyQueue (arg, done) {
  ...
  pump(outgoing, through.obj(function (data, enc, next) {
    ...
    persistence.outgoingUpdateMessageId(client, packet, emptyQueueFilter)
  }), done)
}
