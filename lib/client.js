...
class Client {
  constructor (broker, conn, req) {
    ...
    // CONNECT. Allocated once here — like its inbound sibling `_inboundTopicAliases`
    // above — rather than lazily in connect.js: write()'s hot path then always reads
    // a Map (never null↔Map polymorphism), and teardown `.clear()`s it instead of
    // hand-maintaining a "max > 0 ⇒ Map" invariant. `_outboundTopicAliasMaximum`
    // stays the on/off switch: 0 (the default) means write() short-circuits before
    // it ever touches the map, so an empty map on a non-aliasing client is inert.
    this._outboundTopicAliasMaximum = 0
    this._outboundTopicAliases = new Map()
    // One-shot latch for the `outboundTopicAliasExhausted` telemetry event: set the
    // first time a new topic can't be aliased because the per-connection table is
    // full, so the event fires once per connection rather than per PUBLISH.
    this._outboundTopicAliasExhausted = false
  ...
  close (done) {
    ...
    // can still reach write() in this window. Zero the outbound MAXIMUM first so
    // withOutboundTopicAlias short-circuits on `!max` before it touches the table,
    // then `.clear()` both maps (kept as Maps, never nulled — a .get() on an empty
    // map safely returns undefined, so an in-flight write can't throw on a stale
    // reference). Mirrors the inbound map's teardown exactly.
    this._outboundTopicAliasMaximum = 0
    this._outboundTopicAliases.clear()
