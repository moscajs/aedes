...
class Client {
      ...
      if (that._parsingBatch <= 0) {
        ...
        if (buf) {
          ...
          if (max > 0 && pending && pending.length > 0 &&
              packetWireSize(pending.length) > max) {
            client.rejectPacketTooLarge(packetWireSize(pending.length))
          }
        }
      }
    }
  ...
  disconnect (opts, done) {
    ...
    // takeover, 0x8B shutdown, 0x95 packet-too-large) from a normal drop.
  ...
  rejectPacketTooLarge (size, done) {
    // Latch: the read-path guard can re-enter on each chunk of a multi-chunk
    // oversized body before the async disconnect/close detaches the reader, so
    // bail if we are already rejecting to avoid duplicated clientError/work.
