...
function Client (broker, conn) {
  ...
  this._keepaliveFunc = function () {
    that.emit('error', new Error('keep alive timeout'))
  }
...
function process (client, packet, done) {
  ...
  if (packet.cmd !== 'connect' && client._keepaliveInterval > 0) {
    clearTimeout(client._keepaliveTimer)
    client._keepaliveTimer = setTimeout(client._keepaliveFunc, client._keepaliveInterval)
  }
