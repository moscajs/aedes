...
function handleConnect(client, packet, done) {
  ...
  if (!packet.clean) {
    client.broker.persistence.subscriptionsByClient(client, function(err, subs) {
      if (err) {
      ...
      if (subs) {
        handleSubscribe(client, { subscriptions: subs }, function() {
          doConnack(client, true, done)
        })
      } else {
        doConnack(client, false, done)
      }
    })
  } else {
    doConnack(client, false, done)
...
function doConnack(client, sessionPresent, done) {
  write(client, {
      cmd: 'connack'
    , returnCode: 0
...
function restoreSubs(err, subs, client) {
  if (err) {
    return client.emit('error', err)
  }
