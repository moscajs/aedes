...
function init (client, packet, done) {
  ...
  // Just the on/off switch + effective cap; the Map itself is allocated once in the
  // Client constructor and stays empty until write() assigns the first alias. `max`
  // is min(client's advertised Topic Alias Maximum, broker option), 0 when either is
  // absent (feature off for this connection).
  ...
  // Assign `undefined` rather than `delete`, matching the sibling scrubs at
  // publish.js:180 / write.js:29: the will `properties` object is reference-shared
  // into every fanout packet, and `delete` would transition it into dictionary mode,
  // deopting property access for all recipients.
  if (client._will?.properties?.topicAlias !== undefined) {
    client._will.properties.topicAlias = undefined
  }
