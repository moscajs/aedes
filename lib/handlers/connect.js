...
import { SESSION_NEVER_EXPIRES, ReasonCodes } from '../constants.js'
...
class Connack {
  constructor (arg, version) {
    ...
    if (version === 5) {
      ...
      this.reasonCode = arg.reasonCode ?? connackReasonCodes[arg.returnCode] ?? 0x80
      if (arg.properties) {
...
function init (client, packet, done) {
  ...
  if (packet.protocolVersion === 5 && packet.properties?.receiveMaximum === 0) {
    const error = new Error('Receive Maximum must not be 0')
    doConnack(
      { client, returnCode: 0, reasonCode: ReasonCodes.PROTOCOL_ERROR, sessionPresent: false, version: 5 },
      done.bind(this, error))
    return
  }
  ...
  if (packet.protocolVersion === 5 && packet.properties?.authenticationData !== undefined &&
    ...
    const error = new Error('authentication data without authentication method')
    doConnack(
      { client, returnCode: 0, reasonCode: ReasonCodes.PROTOCOL_ERROR, sessionPresent: false, version: 5 },
      done.bind(this, error))
    return
  ...
  if (packet.protocolVersion === 5 && packet.properties?.authenticationMethod !== undefined) {
    const error = new Error('bad authentication method')
    doConnack(
      { client, returnCode: 0, reasonCode: ReasonCodes.BAD_AUTHENTICATION_METHOD, sessionPresent: false, version: 5 },
      done.bind(this, error))
    return
  }
...
function doConnack (arg, done) {
  ...
  if (version === 5 && arg.returnCode === 0 && !arg.reasonCode && !arg.properties) {
