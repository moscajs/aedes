...
// MQTT 5.0 UNSUBACK reason code for a topic the client was not subscribed to
// (success is the shared ReasonCodes.SUCCESS).
const UNSUB_NO_SUBSCRIPTION_EXISTED = 0x11
...
function actualUnsubscribe (client, packet, done) {
  ...
  const granted = packet.unsubscriptions.map(topic =>
    client.subscriptions[topic] !== undefined ? ReasonCodes.SUCCESS : UNSUB_NO_SUBSCRIPTION_EXISTED)
