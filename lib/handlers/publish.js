...
function resolveTopicAlias (client, packet) {
  ...
  // A duplicated Topic Alias property decodes to an array (mqtt-packet), and
  // `[1,2] < 1` / `> max` are both false via NaN — so a non-integer would slip the
  // range check below and `.set()` a fresh array key per PUBLISH, growing the map
  // without bound (bypassing topicAliasMaximum entirely). Require an integer first;
  // this is the inbound sibling of the guard connect.js applies to the outbound max.
  if (!Number.isInteger(alias) || alias < 1 || alias > max) {
