...
function handle (client, packet, done) {
  ...
  switch (packet.cmd) {
    ...
    case 'pingreq':
      ...
      break
    case 'auth':
      // [#833] MQTT 5.0 Enhanced Authentication continuation / re-authentication.
      handleAuth(client, packet, done)
      break
    case 'disconnect':
      ...
      if (client.version === 5 && packet.properties?.sessionExpiryInterval !== undefined) {
        // [MQTT-3.14.4-3] If the CONNECT declared a Session Expiry Interval of 0,
        // a non-zero value on DISCONNECT is a Protocol Error — an ephemeral
        // session must not be turned persistent at disconnect. Ignore the invalid
        // value (the connection is closing anyway) and surface it on clientError.
        if (client._connectSessionExpiryInterval === 0 && packet.properties.sessionExpiryInterval > 0) {
          client.broker.emit('clientError', client,
            new Error('cannot set a non-zero Session Expiry Interval on DISCONNECT when CONNECT declared 0'))
        } else {
      ...
      if (client.version === 5 && packet.reasonCode === ReasonCodes.DISCONNECT_WITH_WILL) {
        // keep the will: the pending close publishes it
      } else {
