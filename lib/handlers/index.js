...
function handle (client, packet, done) {
  ...
  if (maximumPacketSize > 0 && packetWireSize(packet.length) > maximumPacketSize) {
    client.rejectPacketTooLarge(done)
    return
  }
  ...
  switch (packet.cmd) {
    ...
    case 'disconnect':
      ...
      if (client.version === 5 && packet.properties?.sessionExpiryInterval !== undefined) {
        client.sessionExpiryInterval = packet.properties.sessionExpiryInterval
